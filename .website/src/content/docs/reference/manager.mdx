...
- Admin verbs run nothing and the producer runs nothing. A deployment that
  only produces accumulates partitions; `ProducerHandle.Register` logs
  [SQL0063](/errors/SQL0063/) naming the unclaimed rows, and the one-minute
  `worker_liveness` check keeps an alert active on `__system.alerts` until
  the rows are claimed again.
