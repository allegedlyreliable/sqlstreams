...
| verb | returns | notes |
| --- | --- | --- |
| `Register(ctx, cfg)` | `*ProducerInstance[T]` | resolves the stream, `ErrStreamNotFound`; `nil` cfg is the defaults; logs [SQL0063](/errors/SQL0063/) when the stream's upkeep workers have no live instance |
...
- `ConcurrencyExclusive` in `Message` without a `MessageKey` errors at
  produce time.
- A deployment that only produces runs no upkeep. Its `Register` logs
  [SQL0063](/errors/SQL0063/) naming the unclaimed rows; run
  `sqlstreams manager run` beside it ([Manager](/reference/manager/)).
